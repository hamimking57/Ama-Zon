...
import { TrendingUp, TrendingDown, Bitcoin, Atom, Cpu, Gem, CircleDot, Zap, Brain } from 'lucide-react';
...
const getIcon = (type: AssetType) => {
  switch (type) {
    ...
    case AssetType.FUSION_ENERGY: return <Zap className="text-emerald-400 animate-pulse" size={32} />;
    case AssetType.NEURAL_LINK: return <Brain className="text-rose-400" size={32} />;
    ...
    <div className="bg-slate-900 border border-white/10 rounded-3xl p-6 shadow-2xl hover:border-gold-500/50 transition-all duration-500 group">
      ...
      <div className="flex justify-between items-end mb-6">
        ...
        <div className="text-right">
          ...
          <p className="text-lg font-bold text-gold-500">{(userOwnedAmount || 0).toFixed(asset.type === AssetType.ANTIMATTER ? 6 : 2)}</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <button onClick={() => onSell(asset)} disabled={(userOwnedAmount || 0) <= 0} className="py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold transition-all disabled:opacity-20">Sell</button>
