...
export const DepositModal: React.FC<DepositModalProps> = ({ 
  ...
  const renderGatewayLogo = (gw: PaymentGateway, isSelected: boolean) => {
    if (gw.logoUrl) {
      return (
        <div className={`w-10 h-10 rounded-xl overflow-hidden flex items-center justify-center bg-black/50 border ${isSelected ? 'border-gold-500/50' : 'border-white/5'}`}>
    ...
    if (n.includes('card') || n.includes('bank')) return <CreditCard size={24} className={colorClass} />;
    if (n.includes('crypto') || n.includes('globe') || n.includes('link')) return <Globe size={24} className={colorClass} />;
    return <Zap size={24} className={colorClass} />;
  };
        ...
        <div className="flex flex-col md:flex-row h-full">
          ...
          <div className="flex-grow p-12 max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-center mb-10">
              <div>
                ...
                <div className="flex items-center space-x-2 mt-2">
                   ...
                   <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Portfolio Ready: ${currentBalance.toLocaleString()}</p>
                </div>
              </div>
            ...
            <div className="space-y-8">
                ...
                <div className="flex overflow-x-auto space-x-4 pb-4 custom-scrollbar">
                  {activeGateways.map((gw) => (
                    <button 
                      ...
                      className={`flex-shrink-0 min-w-[130px] p-6 rounded-[2rem] border transition-all flex flex-col items-center space-y-4 ${selectedMethod === gw.name ? 'bg-gold-500/10 border-gold-500 shadow-lg' : 'bg-black border-white/5 hover:border-white/20'}`}
                    >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-black border border-white/10 rounded-[2.5rem] px-14 py-7 text-3xl font-mono text-white focus:border-gold-500 outline-none transition-all shadow-inner"
                    />
                  </div>
                </div>
                ...
                {currentGateway && (
                  <div className="flex flex-col justify-center space-y-3 p-6 bg-black/40 rounded-[2rem] border border-white/5">
                    ...
                    <div className="flex items-center justify-between text-[10px] uppercase font-black tracking-widest">
                       ...
                       <span className="text-gold-500">{currentGateway.merchantName}</span>
                    </div>
                    <div className="flex items-center justify-between text-[10px] uppercase font-black tracking-widest">
                       <span className="text-slate-600">Fee:</span>
                       <span className="text-red-400">{currentGateway.feePercent || 0}% Settlement Cost</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              {currentGateway && (
                <div className="p-8 bg-[#0b101b] border border-gold-500/20 rounded-[3rem] space-y-6 relative overflow-hidden group">
                  <div className="absolute -top-10 -right-10 w-40 h-40 bg-gold-500/5 blur-[50px] pointer-events-none"></div>
                  
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3 text-[10px] font-black uppercase tracking-widest text-gold-500">
                      <Building2 size={16} />
                      <span>Company Receiving Account</span>
                    </div>
                    {currentGateway.link && (
                      <a href={currentGateway.link} target="_blank" rel="noopener noreferrer" className="text-[10px] font-black uppercase text-gold-500 hover:text-white flex items-center transition-colors">
                        Quick Link <LinkIcon size={12} className="ml-2"/>
                      </a>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    <div className="flex items-center space-x-4">
                      <div className="w-14 h-14 rounded-2xl bg-black border border-white/5 flex items-center justify-center overflow-hidden flex-shrink-0">
                         {currentGateway.logoUrl ? (
                           <img src={currentGateway.logoUrl} alt={currentGateway.bankName} className="w-full h-full object-contain p-1" />
                         ) : (
                           <Building2 size={24} className="text-slate-700" />
                         )}
                      </div>
                      <div className="space-y-1">
                        <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Official Bank/Provider</p>
                        <p className="text-xl font-bold text-white">{currentGateway.bankName || currentGateway.name}</p>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[9px] text-slate-500 font-black uppercase tracking-widest">Account # / Address</p>
                      <div className="flex items-center justify-between bg-black/50 p-3 rounded-xl border border-white/5">
                        <span className="text-lg font-mono font-bold text-white truncate mr-4">{currentGateway.accountNumber}</span>
                        <button 
                          onClick={() => {
                          ...
                          className="text-[9px] font-black uppercase bg-gold-500 text-black px-4 py-2 rounded-lg active:scale-95 transition-all hover:bg-gold-400"
                        >
                          Copy
                        </button>
                      </div>
              ...
              {/* Reference Input */}
              <div>
                <label className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] mb-4 block">Verification ID (Receipt Reference)</label>
                <div className="relative">
                  <input 
                    type="text" 
                    placeholder="Paste transaction reference or hash..."
                    value={reference}
                    onChange={(e) => setReference(e.target.value)}
                    className="w-full bg-black border border-white/10 rounded-[1.5rem] px-6 py-5 text-sm font-mono text-white focus:border-gold-500 outline-none transition-all placeholder:text-slate-800"
                  />
              ...
              <div className="flex items-center space-x-4 pt-4">
                <button 
                  ...
                  className="flex-grow py-7 bg-gold-500 text-black font-black rounded-[2.5rem] shadow-[0_0_50px_rgba(234,179,8,0.3)] hover:bg-gold-400 hover:scale-[1.02] transition-all flex items-center justify-center space-x-4 disabled:opacity-50 active:scale-95"
                >
                  {isProcessing ? (
                    <div className="w-8 h-8 border-4 border-black border-t-transparent rounded-full animate-spin"></div>
                  ) : (
                    <>
                      <span className="uppercase tracking-[0.2em] text-sm font-black">Secure Submission</span>
              ...
              <div className="flex items-center justify-center space-x-3 text-[10px] text-slate-600 font-black uppercase tracking-widest">
                 <AlertCircle size={14} />
                 <span>Verified Settlement Only</span>
              </div>
            </div>
          </div>
        </div>
